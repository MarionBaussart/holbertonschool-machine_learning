...
def posterior(x, n, P, Pr):
    ...
    Args:
        ...
        P: 1D numpy.ndarray containing the various hypothetical
            probabilities of developing severe side effects
        Pr: 1D numpy.ndarray containing the prior beliefs of P
    Return: the posterior probability of each probability in P given x and n
    """
    if type(n) != int or n <= 0:
        raise ValueError('n must be a positive integer')
    if type(x) != int or x < 0:
    ...
    likelihood_P = likelihood(x, n, P)
    intersection = likelihood_P * Pr
    marginal = np.sum(intersection)

    posterior = likelihood_P * Pr / marginal
