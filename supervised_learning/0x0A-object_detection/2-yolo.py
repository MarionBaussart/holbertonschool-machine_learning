...
class Yolo:
    ...
    def filter_boxes(self, boxes, box_confidences, box_class_probs):
        ...
        for i, box in enumerate(boxes):
            ...
            max_box_score = np.max(box_score, axis=-1)
            max_classe = np.argmax(box_score, axis=-1)

            index = np.where(max_box_score >= self.class_t)

            filtered_boxes.append(box[index])
            box_classes.append(max_classe[index])
            box_scores.append(max_box_score[index])
